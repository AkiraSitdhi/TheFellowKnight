...
public class Arrow extends Projectile implements IRenderable {

	public Arrow(int damage, int timer, Coordinate coordinate, Coordinate destination) {
	...
	public void draw(GraphicsContext gc) {
		// TODO Auto-generated method stub
		gc.drawImage(rotateImage(ImageHolder.arrowSprite, angle), getCoordinate().getX(), getCoordinate().getY());
	}
		...
		for (int e = 0; e < entity.size(); e++) {
			if (entity.get(e) instanceof CurrentEnemy) {
				...
				for (int i = 0; i < target.getEnemyList().size(); i++) {
					if (Math.hypot(coordinate.getX() - target.getEnemyCoordinateList().get(i).getX(), coordinate.getY()
							- target.getEnemyCoordinateList().get(i).getY()) <= GameUnit.UNIT_SIZE / 2) {
